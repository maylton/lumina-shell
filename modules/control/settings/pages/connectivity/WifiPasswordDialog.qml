...
Item {
    ...
    readonly property string networkName: network
        ...
        : ""
    readonly property bool opened: passwordPopup.opened
    ...
    Controls.Popup {
        ...
        parent: root.parent
        x: Math.max(16, (root.availableWidth - width) / 2)
        y: Math.max(16, (root.availableHeight - height) / 2)
        width: Math.max(320, Math.min(580, root.availableWidth - 32))
        height: Math.max(360, Math.min(400, root.availableHeight - 32))
        padding: 0
        modal: true
        focus: true
